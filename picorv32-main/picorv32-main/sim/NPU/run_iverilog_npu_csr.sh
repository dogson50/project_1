...
"${iverilog_bin}" -g2012 -f sim/NPU/filelist_npu_csr.f -o .sim/tb_npu_csr_if.vvp

if [[ "${wave}" -eq 1 ]]; then
  "${vvp_bin}" -N .sim/tb_npu_csr_if.vvp +vcd
  echo "Wave dumped to ${repo_root}/tb_npu_csr_if.vcd"
else
  "${vvp_bin}" -N .sim/tb_npu_csr_if.vvp
fi
