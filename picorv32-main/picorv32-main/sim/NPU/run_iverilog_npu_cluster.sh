...
"${iverilog_bin}" -g2012 -f sim/NPU/filelist_npu_cluster.f -o .sim/tb_npu_core_cluster.vvp

if [[ "${wave}" -eq 1 ]]; then
  "${vvp_bin}" -N .sim/tb_npu_core_cluster.vvp +vcd
  echo "Wave dumped to ${repo_root}/tb_npu_core_cluster.vcd"
else
  "${vvp_bin}" -N .sim/tb_npu_core_cluster.vvp
fi
